...
class Dopplershift_data:
    ...
    _noise_ch = 'TODO'
    _ref_channel = 'TODO'
    _reference = 'TODO' # 'diode' or 'hene'
    diode_wavenumber = 'TODO' # /cm
    hene_wavenumber = 'TODO' # /cm
    ...
    _devices = {"tagger": ['timestamp', 'offset', 'bunch_no', 'events_per_bunch', 'channel', 'delta_t'], 
                ...
                "powermeter_1": ['timestamp', 'offset', 'powermeter_1'],
    ...
    _transition_wavenumber = 'TODO' # /cm
    ...
    _masses = 'TODO' # should be a dictionary with key = mass number and value = exact mass in amu, e.g., for Ag-109 -> {109:108.904752}
    _DATA_FOLDER = 'TODO' # e.g., 'C:\\Users\\u0148746\\OneDrive - KU Leuven\\Documents\\PhD\\CRIS\\Ag_6-22\\Online Ag\\Data\\'
    _SAVEPATH = 'TODO' # e.g., 'C:\\Users\\u0148746\\OneDrive - KU Leuven\\Documents\\PhD\\CRIS\\Ag_6-22\\Online Ag\\Analysis_output\\'
    _SAVEPATH_FIG = 'TODO' # e.g., 'C:\\Users\\u0148746\\OneDrive - KU Leuven\\Documents\\PhD\\CRIS\\Ag_6-22\\Online Ag\\Fig_analysis_output\\'
    ...
    def checkClassVariables(self) -> None:
        '''Checks if you defined all neccesary class variables'''
        class_var = [self._signal_ch,self._noise_ch,self._ref_channel,self.diode_wavenumber,self.hene_wavenumber,self.bin_size_voltage,self.bin_size_MHz,self._transition_wavenumber,self._masses,self._DATA_FOLDER,self._SAVEPATH,self._SAVEPATH_FIG]
        for var in class_var:
            if var == 'TODO':
                raise Exception(f'Define all the class variables that have TODO in Dopplershift_analysis.py')
    ...
    def gate_tof(self, data: pd.DataFrame, manual: Union[list,bool] = False) -> pd.DataFrame:
        ...
        if manual:
            ...
            plt.show()
            data.loc[((data['delta_t'] < manual[1]) | (data['delta_t'] > manual[0])), 'delta_t'] = -0.005
            data.loc[((data['delta_t'] < manual[1]) | (data['delta_t'] > manual[0])), 'channel'] = -1
        ...
        indexrightbound = np.array(np.where(TOF_spectrum[1][indexmax:] < (1-0.9) * max(tof)))[0][0] + indexmax
